...
import DetailedPendingLayout from "../Layouts/DetailedPendingLayout"; // If you have a dedicated Verified layout, replace
...
const DetailedVerified_Adventures = () => {
  ...
  // Redux verified adventures list
  const verifiedAdventures = useSelector(
    (store) => store.adventures.verified
  );

  // Priority: data from navigation state, fallback to Redux
  const adventure =
    state?.adventure ||
    verifiedAdventures.find((a) => String(a.id) === id) ||
    verifiedAdventures.find((a) => String(a.id) === `#${id}`);
  ...
  // Gallery images fallback logic (same style as your Pending file)
  ...
  return (
    <DetailedPendingLayout
      ...
      galleryImages={galleryImages}
      // No approve/reject for verified adventures
      onApprove={null}
      onReject={null}
    />
  );
